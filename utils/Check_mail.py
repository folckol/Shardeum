            ...
            with MailBox(provider).login(mail, psw) as mailbox:
                for msg in mailbox.fetch(limit=20, reverse=True):  # Only fetch the most recent 20 emails
                    ...
                    if msg.from_ == _from:
                        ...
                        verification_link_tag = soup.find('a', {'style': 'border-radius:500px;padding:19px 20px;font-size:16px;letter-spacing: 1px;display:inline-block;color:#ffffff;text-decoration:none;outline:0px;line-height:1;box-sizing:border-box;width:auto'})
                        verification_link = verification_link_tag['href'] if verification_link_tag else None
